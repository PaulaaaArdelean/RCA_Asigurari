...
namespace RCA_Asigurari.Models
{
    public class OfertaPF
    {
        ...
        [RegularExpression(@"^[1-8][0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[1-2][0-9]|3[0-1])(0[1-9]|[1-4][0-9]|5[0-2])[0-9]{4}$", ErrorMessage = "CNP-ul introdus de dumneavoastră este invalid. ")]
        public string CNP { get; set; }


        [Display(Name = "Serie CI")]
        [RegularExpression("^[A-Z]{2}$", ErrorMessage = "Seria CI este incorectă")]
        public string SerieCI { get; set; }

        [Display(Name = "Numar CI")]
        [RegularExpression("^[0-9]{6}$", ErrorMessage = "Numărul cărții de identitate trebuie să conțina 6 cifre")]
        public string NumarCI { get; set; }

        [Display(Name = "Vârsta")]
        [RegularExpression("^(1[8-9]|[2-9][0-9])$", ErrorMessage = "Câmpul este completat incorect.")]
